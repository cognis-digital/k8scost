...
**k8scost** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `k8scost` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
k8scost ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
k8scost ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
k8scost ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
```
...
works whatever `k8scost` outputs:

```python
from cognis_connect import normalize, misp
findings = [normalize(rec, source="k8scost") for rec in records]   # records = your JSON output
