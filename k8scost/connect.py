"""Native cognis-connect emit for k8scost — forward findings to any platform.

Maps k8scost's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    k8scost ... --format json | k8scost-emit --to stix
    k8scost-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        ...
        out.pop('savings', None)
        out.pop('findings_count', None)
        out.pop('total_findings', None)
        out.pop('monthly_cost', None)
        out.pop('recommended_monthly_cost', None)
        out.pop('monthly_savings', None)
        out.pop('savings_pct', None)
        out['title'] = rec.get('name', '')
        out['severity'] = 'low'
        out['type'] = 'cost'
        out['description'] = 'Cost optimization opportunity'
        out['tags'] = ['cost']
